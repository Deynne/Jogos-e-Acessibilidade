...
public class FightManager : MonoBehaviour
{
    ...
    private Dictionary<string,AudioClip> numberClipMap = new Dictionary<string,AudioClip>();
    ...
    public void PlayHPCounter(Fighter fghtr) {
        AudioClip tempClip;
        if(numberClipMap.ContainsKey(fghtr.hP.ToString()))
            tempClip = numberClipMap[fghtr.hP.ToString()];
        else {
            tempClip = Resources.Load<AudioClip>(TextPlayer.SONS_NUMEROS + fghtr.hP);
            numberClipMap.Add(fghtr.hP.ToString(),tempClip);
        }
        if(fghtr.player) {
            if(fghtr.hP != 1) {
    ...
    public IEnumerator PlayGameStatus() {
        ...
        AudioClip tempClip;
        if(numberClipMap.ContainsKey(rodadaAtual.ToString()))
            tempClip = numberClipMap[rodadaAtual.ToString()];
        else {
        ...
        TextPlayer.instance.playInSequence  (listaSons.rodada, tempClip);
